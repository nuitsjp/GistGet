...
public class SyncPlan
{
    ...
    /// インストール対象のパッケージリスト
    /// </summary>
    public List<PackageDefinition> ToInstall { get; set; } = new();

    /// <summary>
    /// アンインストール対象のパッケージリスト（Uninstallフラグがtrueのもの）
    /// </summary>
    public List<PackageDefinition> ToUninstall { get; set; } = new();

    /// <summary>
    /// 既にインストール済みのパッケージリスト（冪等性確保のためスキップ）
    /// </summary>
    public List<PackageDefinition> AlreadyInstalled { get; set; } = new();

    /// <summary>
    /// Gist定義に存在するが、WinGetカタログで見つからないパッケージリスト
    /// </summary>
    public List<PackageDefinition> NotFound { get; set; } = new();
