...
public class GistInputService
{
    ...
    public void ValidateFileName(string fileName)
    {
        ...
        if (fileName.IndexOfAny(InvalidFileNameChars) >= 0)
            throw new ArgumentException(
                $"File name contains invalid characters. Invalid characters: {string.Join(", ", InvalidFileNameChars)}",
                nameof(fileName));
        ...
        if (ReservedFileNames.Contains(fileNameWithoutExtension))
            throw new ArgumentException($"File name '{fileName}' is a reserved name and cannot be used",
                nameof(fileName));

        // ピリオドで終わるファイル名は無効
        if (fileName.EndsWith('.')) throw new ArgumentException("File name cannot end with a period", nameof(fileName));
    }
    ...
    public string ExtractGistIdFromUrl(string input)
    {
        ...
        if (!match.Success) throw new ArgumentException("Invalid Gist ID or URL format", nameof(input));
