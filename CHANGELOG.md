...
- 新規OSで `sync` 実行時に「ピン留め済みパッケージを取得中...」でハングする問題を修正（`winget pin list` に `--accept-source-agreements` と `--disable-interactivity` を付与）

## [1.4.1] - 2026-01-24

### Fixed

- `upgrade --all` で適用不可・バージョン不明なパッケージを除外
...
- `winget export` コマンドのパススルーを追加
- `winget import` コマンドのパススルーを追加
- `upgrade` コマンドにwingetの不足オプションを追加
...
[Unreleased]: https://github.com/nuitsjp/GistGet/compare/v1.4.1...HEAD
[1.4.1]: https://github.com/nuitsjp/GistGet/compare/v1.4.0...v1.4.1
